...
public class AppMockTest {
	...
	private String obtainAccessToken(String username, String password) throws Exception {
		...
		String content = mockMvc
				.perform(post("/users/login").header("Authorization", authorization).contentType(contentType)
	...
	private HttpHeaders createHeader() throws Exception {

		String contenido = this.obtainAccessToken("rcarballo75@emergya.com", "Emergya");
		Assert.assertFalse(StringUtils.isEmpty(contenido));

		String[] datosLogin = contenido.split(",");

		String[] token = datosLogin[0].split(":");

		String authorization = token[1].replaceAll("\"", "");
	...
	public void setup() throws Exception {
		...
		try {
			...
			User usuarioA = new User("rcarballo", bCryptPasswordEncoder.encode("Emergya"), "Carballo",
					"rcarballo@emergya.com");
			// usuarioA.addAssets(assetsList.get(0));
			// usuarioA.addRole(roleList.get(0));
			this.userList.add(userRepository.save(usuarioA));
			this.userList.add(userRepository.save(new User("rcarballo75", bCryptPasswordEncoder.encode("Emergya"),
					"Carballo", "rcarballo75@emergya.com")));
	...
	public void postUsersTest() throws Exception {
		String url = "/users/users";
		String json = "{\"name\": \"User 1\",\"surname\": \"Example\",\"email\": \"email@example.com\",\"date_of_birth\": \"1990-02-26\",\"password\": \"dm1m2'%cma1g64$4&1\",\"gender\": \"female\",\"assets\": [{\"id\":\""
				+ this.assetsList.get(0).getId() + "\"}],\"roles\": [{\"id\":\"" + this.roleList.get(0).getId()
				+ "\"}]}";
		mockMvc.perform(post(url).content(json).contentType(contentType)).andExpect(status().isCreated());
	}
	...
	public void userLogoutTest() throws Exception {
		...
		mockMvc.perform(get(url).headers(createHeader())).andExpect(status().isOk());
	}
	
	@Test
	public void userLogoutFailTest() throws Exception {
		String url = "/users/users/logout";
	...
	public void postRoleTest() throws Exception {
		...
		mockMvc.perform(post(url).content(role_new_json).contentType(contentType)).andExpect(status().isCreated());
	}

	@Test
	public void postRoleFailureTest() throws Exception {
		String url = "/users/roles";
		mockMvc.perform(post(url).content(role_complete_new_json).contentType(contentType))
	...
	public void postResourcesRolesTest() throws Exception {
		...
		mockMvc.perform(put(url).content(role_resources_new_json).contentType(contentType)).andExpect(status().isOk());
	}
	...
	public void updateRoleTest() throws Exception {
		...
		mockMvc.perform(put(url).content(role_complete_update_json).contentType(contentType))
	...
	public void deleteRoleTest() throws Exception {
		...
		mockMvc.perform(delete(url).contentType(contentType)).andExpect(status().isOk());
	}
	...
	public void updateUserRolesTest() throws Exception {
		...
		mockMvc.perform(put(url).content(update_user_roles_json).contentType(contentType)).andExpect(status().isOk());
	}
	...
	public void deleteUserRolesTest() throws Exception {
		...
		mockMvc.perform(delete(url).content(update_user_roles_json).contentType(contentType))
				.andExpect(status().isOk());
	}
	...
	public void checkRoleUserTest() throws Exception {
		...
		mockMvc.perform(get(url).contentType(contentType)).andExpect((status().is2xxSuccessful()));
	}
	...
	public void postAssetTest() throws Exception {
		String url = "/users/assets";
		String json = "{\"name\": \"Asset JJJ\",\"description\": \"none\",\"type\": \"Building\",\"parents\": [{\"id\":\""
				+ assetsList.get(0).getId() + "\"}],\"childrens\": [{\"id\":\"" + assetsList.get(1).getId() + "\"}]}";
		mockMvc.perform(post(url).content(json).contentType(contentType)).andExpect(status().isCreated());
	}
	...
	public void updateAssetTest() throws Exception {
		...
		String json = "{\"name\": \"Asset CUCU\",\"description\": \"none\",\"type\": \"Building\",\"parents\": [{\"id\":\""
				+ assetsList.get(2).getId() + "\"}],\"childrens\": [{\"id\":\"" + assetsList.get(2).getId() + "\"}]}";
		mockMvc.perform(put(url).content(json).contentType(contentType)).andExpect(status().isOk());
	}
	...
	public void checkAssetFailure() throws Exception {
		...
		mockMvc.perform(get(url).contentType(contentType)).andExpect(status().isUnprocessableEntity());
	}
	...
	public void updateUserTest() throws Exception {
		String url = "/users/users/" + this.userList.get(0).getId();
		String json = "{\"name\": \"User 1\",\"surname\": \"Example\",\"email\": \"email@example.com\",\"date_of_birth\": \"1990-02-26\",\"password\": \"dm1m2'%cma1g64$4&1\",\"gender\": \"female\",\"assets\": [{\"id\":\""
				+ this.assetsList.get(0).getId() + "\"}],\"roles\": []}";
		mockMvc.perform(put(url).content(json).contentType(contentType)).andExpect(status().isOk());
	...
	public void changePasswordFailTest() throws Exception {
		...
		mockMvc.perform(put(url).content(json).contentType(contentType)).andExpect(status().isUnprocessableEntity());
	}
	...
	@Test
	public void getUserMeTest() throws Exception {
	...
	@Test
	public void getUserMeFailTest() throws Exception {
		String url = "/users/users/me";
